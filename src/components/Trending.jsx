...
const featured = {
  ...
  desc: 'An FBI agent uncovers a mole threatening national security. Binge the entire season now in 4K Ultra HD.',
  rating: '9.2',
  year: '2026',
  genre: 'Action • Thriller',
  img: 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=1400&q=80',
}
...
const continueWatching = [
  { title: 'Peaky Blinders', progress: 72, ep: 'S6 E4', img: 'https://images.unsplash.com/photo-1574375927938-d5a98e8d7e28?w=400&q=80' },
...
export default function Trending() {
  return (
    <section className="relative bg-[#0a0a0a] overflow-hidden">
          ...
          <div className="w-full max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
            <div className="max-w-2xl">
              <div className="flex items-center gap-3 mb-4">
                <span className="bg-red-600 text-white text-[10px] font-black px-2.5 py-1 rounded tracking-wider">TRENDING #1</span>
              ...
              <p className="text-red-400 font-semibold text-lg mb-3">{featured.subtitle}</p>
              ...
              <div className="flex flex-wrap gap-3">
                <a href="#pricing" className="inline-flex items-center gap-2.5 bg-red-600 hover:bg-red-500 text-white font-bold px-8 py-4 rounded-lg transition-all hover:shadow-[0_8px_40px_rgba(229,9,20,0.4)]">
      ...
      <div className="relative py-16">
        <div className="w-full max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
          <div className="flex items-center justify-between mb-8">
            ...
            <a href="#pricing" className="flex items-center gap-1 text-red-400 text-sm font-semibold hover:text-red-300 transition-colors">
              ...
              <div key={i} className="group cursor-pointer">
                <div className="relative rounded-xl overflow-hidden aspect-[2/3] mb-3 shadow-lg">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <div className="w-14 h-14 rounded-full bg-red-600 flex items-center justify-center shadow-[0_0_30px_rgba(229,9,20,0.6)] scale-75 group-hover:scale-100 transition-transform duration-300">
                ...
                <h4 className="text-white font-bold text-sm group-hover:text-red-400 transition-colors">{movie.title}</h4>
                <p className="text-gray-600 text-xs mt-0.5">{movie.genre}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {continueWatching.map((show, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="relative rounded-xl overflow-hidden aspect-video mb-3 shadow-lg">
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    ...
                    <div className="w-full bg-white/20 h-1">
                      <div className="bg-red-600 h-1" style={{ width: `${show.progress}%` }} />
                    </div>
                  </div>
                ...
                <h4 className="text-white font-bold text-sm group-hover:text-red-400 transition-colors">{show.title}</h4>
                <p className="text-gray-600 text-xs mt-0.5">{show.progress}% watched</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative py-16 bg-gradient-to-b from-transparent via-red-950/5 to-transparent">
        <div className="w-full max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
          <div className="text-center mb-10">
            <h3 className="text-white font-black text-3xl mb-3">
              Stream on <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-400">Any Device</span>
            </h3>
            <p className="text-gray-500 text-base">28,000+ channels available across all your favorite platforms</p>
          </div>

          <div className="flex flex-wrap justify-center gap-6 mb-12">
            {platforms.map((p, i) => (
              <div key={i} className="flex flex-col items-center gap-3 group cursor-pointer">
                <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center group-hover:border-red-500/20 group-hover:bg-red-500/5 transition-all duration-300">
                  <p.icon size={26} className="text-gray-500 group-hover:text-red-400 transition-colors" />
                </div>
                <span className="text-xs text-gray-600 group-hover:text-gray-300 transition-colors font-medium">{p.label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="text-center">
            <a
              href="#pricing"
              className="inline-flex items-center gap-2.5 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold px-10 py-4 rounded-lg hover:from-red-500 hover:to-red-600 hover:shadow-[0_8px_40px_rgba(229,9,20,0.35)] transition-all duration-300 text-lg"
            >
